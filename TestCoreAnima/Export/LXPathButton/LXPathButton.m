...
@interface LXPathButton ()<LXPathCenterButtonDelegate, LXPathItemButtonDelegate>
{
    LXRemindMethod *remind;
}
...
@property (strong, nonatomic) LXPathCenterButton *pathCenterButton;
...
- (void)configureViewsLayout
{
    ...
    //
    _pathCenterButton = [[LXPathCenterButton alloc]initWithImage:self.centerImage highlightedImage:self.centerHighlightedImage];
...
- (void)configureSounds
{
    ...
    remind = [[LXRemindMethod alloc] initSystemSoundWithName:@"tweet_sent" SoundType:@"caf"];
...
//    AudioServicesPlaySystemSound(self.foldSound);
    ...
    for (int i = 1; i <= self.itemButtons.count; i++) {
        ...
        LXPathItemButton *itemButton = self.itemButtons[i - 1];
...
- (void)resizeToFoldedFrame
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        for (LXPathItemButton *itemButton in self.itemButtons) {
...
- (void)pathCenterButtonBloom
{
    ...
    for (int i = 1; i <= self.itemButtons.count; i++) {
        ...
        LXPathItemButton *pathItemButton = self.itemButtons[i - 1];
...
- (void)itemButtonTapped:(LXPathItemButton *)itemButton
{
    if ([_delegate respondsToSelector:@selector(itemButtonTappedAtIndex:)]) {
        ...
        LXPathItemButton *selectedButton = self.itemButtons[itemButton.tag];
        ...
        for (int i = 0; i < self.itemButtons.count; i++) {
            ...
            LXPathItemButton *unselectedButton = self.itemButtons[i];
